...
insert into DetallesFactura(codigoFacturaFK,codigoProductoFK, cantidadProd, precioUnitario)values
(1, 'P1', 5, 3600),(2, 'P2', 12, 3600),(3, 'P3', 8, 3600),(4, 'P4', 17, 3600),(5, 'P5', 24, 3600),(6, 'P6', 9, 3600),
(7, 'P7', 15, 4120),(8, 'P8', 33, 2780),(9, 'P9', 11, 3490),(10, 'P10', 27, 1980),(11, 'P11', 18, 4050),
(12, 'P12', 21, 2210),(13, 'P13', 7, 4870),(14, 'P14', 29, 1340),(15, 'P15', 14, 3670),(16, 'P16', 35, 4590),
(17, 'P17', 19, 1420),(18, 'P18', 6, 2760),(19, 'P19', 38, 3080),(20, 'P20', 16, 3950),(21, 'P21', 20, 1890),
(22, 'P22', 13, 4700),(23, 'P23', 25, 2300),(24, 'P24', 10, 3120),(25, 'P25', 28, 2220),(26, 'P26', 31, 1780),
(27, 'P27', 22, 2460),(28, 'P28', 36, 4810),(29, 'P29', 30, 1550),(30, 'P30', 40, 1970),(31, 'P31', 3, 3880),
(32, 'P32', 26, 1430),(33, 'P33', 4, 4560),(34, 'P34', 2, 4900),(35, 'P35', 32, 3290),(36, 'P36', 23, 1150),
(37, 'P37', 34, 2120),(38, 'P38', 1, 2680),(39, 'P39', 39, 3110),(40, 'P40', 41, 1370),(41, 'P41', 45, 4770),
(42, 'P42', 50, 1990),(43, 'P43', 44, 3620),(44, 'P44', 43, 4810),(45, 'P45', 47, 2700),(46, 'P46', 42, 3810),
(47, 'P47', 46, 1240),(48, 'P48', 48, 3150),(49, 'P49', 49, 1450),(50, 'P50', 37, 2740),(51, 'P51', 6, 1830),
(52, 'P52', 13, 3250),(53, 'P53', 22, 4570),(54, 'P54', 31, 2010),(55, 'P55', 17, 4870),(56, 'P56', 9, 1230),
(57, 'P57', 28, 3860),(58, 'P58', 20, 4720),(59, 'P59', 11, 1550);
...
select * from Venta where docUsuarioFK > 1010;

select nombreProducto, cantidadDisponible 
from Producto join Stock on Producto.codigoProducto = Stock.idProductoFK
where Stock.cantidadDisponible < 10;

-- Clientes con más de una venta
select nombreCliente, apellidoCliente, COUNT(codigoVenta) as 'TotalVentas' 
from Cliente join Venta on Cliente.docCliente = Venta.docClienteFK
group by Cliente.docCliente
having COUNT(Venta.codigoVenta) > 1;
...
# Consulta usuarios de tipo administrador
select * from Usuario where tipoUsuario = 'Administrador';

# Consulta usuarios de con telefono específico
select docUsuario, nombreUsuario, appellidoUsuario from Usuario
 where telefonoUsuario = 3178901234;

# Consulta usuarios con apellido Torres
select nombreUsuario, appellidoUsuario, docUsuario from Usuario 
where appellidoUsuario like '%Torres%';

# Consulta usuarios cuyo documento inicie con 5
select docUsuario,nombreUsuario, appellidoUsuario,estadoUsuario 
from Usuario where docUsuario Like '5%';

# Consulta para saber cuantos usuarios son de tipo empleado
select count(tipoUsuario) from Usuario where tipoUsuario = 'Empleado';

# Consulta usuarios que cuya dirreción contiene avenida
select docUsuario, nombreUsuario, appellidoUsuario from Usuario 
where direccionUsuario not like '%Avenida%';
...
UPDATE Producto SET estadoProducto = false WHERE codigoProducto = 'P1';

UPDATE Stock SET cantidadDisponible = 50 WHERE idProductoFK = 'P2';
...
UPDATE FacturaVenta SET medioPago = 'Tarjeta' WHERE codigoFactura = 1;
...
/** ConsuItas muItitabIas**/
		-- facturas con información del cliente y el usuario que realizó la venta
select codigofactura, totalfactura, nombrecliente, nombreusuario from facturaventa join venta on codigoventafk = codigoventa
join cliente on venta.docclientefk = cliente.doccliente join usuario on venta.docusuariofk = usuario.docusuario;

		-- productos que nunca han sido vendidos
select * from producto
where codigoproducto not in (
    select codigoproductofk from detallesfactura
);

		-- detalles de venta incluyendo producto, cantidad y subtotal
select codigoventa, nombreproducto, cantidadprod, preciounitario, (cantidadprod * preciounitario) as 'subtotal'
from venta join facturaventa on venta.codigoventa = facturaventa.codigoventafk join detallesfactura on facturaventa.codigofactura = detallesfactura.codigofacturafk
join producto on detallesfactura.codigoproductofk = producto.codigoproducto;


	-- ventas realizadas por cada empleado (usuario)
select nombreusuario, count(codigoventa) as 'numventas'from usuario join venta on usuario.docusuario = venta.docusuariofk
group by usuario.nombreusuario;

		-- productos en facturas con estado de pago "pendiente"
select p.nombreproducto, d.cantidadprod, f.estadopago from detallesfactura d join producto p on d.codigoproductofk = p.codigoproducto
join facturaventa f on d.codigofacturafk = f.codigofactura where f.estadopago = 'pendiente';

		-- Facturas con datos del cliente, usuario que vendió y medio de pago
select codigofactura, nombrecliente as 'CIiente', nombreusuario as 'Usuari0', mediopago
from facturaventa join venta on facturaventa.codigoventafk = venta.codigoventa join cliente on venta.docclientefk = cliente.doccliente
join usuario on venta.docusuariofk = usuario.docusuario;

		-- Facturas junto a los productos que incluye y el total de unidades por producto
select codigofactura, nombreproducto, sum(cantidadprod) as 'total unidades' from facturaventa
join detallesfactura on facturaventa.codigofactura = detallesfactura.codigofacturafk
join producto on detallesfactura.codigoproductofk = producto.codigoproducto group by facturaventa.codigofactura, producto.nombreproducto;

		-- Clientes con sus productos comprados más caros
select nombrecliente, nombreproducto, max(preciounitario) as 'pr0duct0 c0m precio mas alto'
from cliente join venta on cliente.doccliente = venta.docclientefk join facturaventa on facturaventa.codigoventafk = venta.codigoventa
join detallesfactura on detallesfactura.codigofacturafk = facturaventa.codigofactura
join producto on detallesfactura.codigoproductofk = producto.codigoproducto
group by cliente.nombrecliente, producto.nombreproducto;

# Subconsulta clientes que tengan una venta en proceso
...
DELIMITER //
CREATE PROCEDURE RegistrarVenta(IN p_docUsuario INT,IN p_docCliente INT,IN p_direccionEnv VARCHAR(100),IN p_fEntrega DATE,
    IN p_fReparto DATE,IN p_estadoVenta BOOL,IN p_precioEnv DOUBLE)
BEGIN
    INSERT INTO Venta (docUsuarioFK,docClienteFK,direccionEnv,fEntrega,fReparto,estadoVenta,precioEnv) 
    VALUES (p_docUsuario, p_docCliente, p_direccionEnv, p_fEntrega, p_fReparto, p_estadoVenta, p_precioEnv);
END //
DELIMITER ;
CALL RegistrarVenta(5214590, 158001, 'Calle 45 #12-34', '2025-04-08', '2025-04-09', TRUE, 5000.00);

        -- Calcular el total de compras de un cliente
delimiter //
create procedure totalcomprascliente(in p_doccliente int)
begin
    select nombrecliente, sum(totalfactura) as 'total comprado'
    from cliente join venta on cliente.doccliente = venta.docclientefk
    join facturaventa on venta.codigoventa = facturaventa.codigoventafk where cliente.doccliente = p_doccliente
    group by cliente.nombrecliente;
end //
delimiter ;
call totalcomprascliente(158001);


		-- Marcar factura como pagada
delimiter //
create procedure marcarfacturapagada(in p_codigofactura int)
begin
    update facturaventa
...
delimiter ;
select*from FacturaVenta;
CALL MarcarFacturaPagada(15);

		-- Insertar producto con stock inicial
delimiter //
create procedure insertarproductoconstock(in p_codigoproducto varchar(20),in p_nombreproducto varchar(50),in p_precioproducto double,
    in p_descripcion text,in p_estado bool,in p_cantidad int,in p_fentrada date)
begin
    insert into producto(codigoproducto, nombreproducto, precioproducto, descripcionproducto, estadoproducto)
    values (p_codigoproducto, p_nombreproducto, p_precioproducto, p_descripcion, p_estado);
    insert into stock(idproductofk, cantidaddisponible, estadostock, fentradaproducto)
    values (p_codigoproducto, p_cantidad, p_estado, p_fentrada);
end //
delimiter ;
call insertarproductoconstock('p88', 'teclado mecánico', 120000.00, 'rgb y switch azul', true, 20, '2025-04-08');

		-- Detalle de factura completa por código
delimiter //
create procedure detallefacturacompleta(in p_codigofactura int)
begin
    select codigofactura, totalfactura, fechaemision, estadopago,nombrecliente, nombreproducto, cantidadprod, preciounitario
    from facturaventa join venta on facturaventa.codigoventafk = venta.codigoventa
    join cliente on venta.docclientefk = cliente.doccliente
    join detallesfactura on facturaventa.codigofactura = detallesfactura.codigofacturafk
    join producto on detallesfactura.codigoproductofk = producto.codigoproducto
    where facturaventa.codigofactura = p_codigofactura;
end //
delimiter ;
select*from facturaventa;
call detallefacturacompleta(1);

# Procedimiento que actualiza la dirección de un usuario
DELIMITER //
create procedure actualizarDireccionUs(in docUs int, in dirUs varchar(100))
Begin
	Update Usuario set direccionUsuario = dirUs where docUsuario = docUs;
end//
DELIMITER ;
call actualizarDireccionUs(109234596,'Calle 45 #67-89, Bogotá');

# Procedimiento que inactiva un usuario dado su documento
DELIMITER //
...
select doccliente, nombrecliente, apellidocliente, sum(totalfactura) as 'total comprado'
from cliente join venta on cliente.doccliente = venta.docclientefk
join facturaventa on venta.codigoventa = facturaventa.codigoventafk
group by cliente.doccliente;
...
/*TRIGGERS*/
		-- Disminuir stock cuando se inserta un detalle de factura
delimiter //
...
begin
    ...
    where idproductofk = new.codigoproductofk;
end //
delimiter ;
select*from detallesfactura;
insert into detallesfactura (codigofacturafk, codigoproductofk, cantidadprod, preciounitario)
values (2, 'P1', 2, 10.00);
-- ver el stock:
select * from stock where idproductofk = 'P1';

		-- Registrar cambios de estado en productos para tener un registr0 en Ia empresa
...
for each row
begin
    if old.estadoproducto <> new.estadoproducto then
        insert into historialestadoproducto (codigoproductofk, estadoanterior, estadonuevo) values (old.codigoproducto, old.estadoproducto, new.estadoproducto);
    end if;
end //
delimiter ;
update producto set estadoproducto = false where codigoproducto = 'P1';
-- consultar el historial
select * from historialestadoproducto where codigoproductofk = 'P1';

		-- desactivar automáticamente una venta entregada
delimiter //
create trigger trg_autodesactivar_venta
before update on venta
for each row
begin
    if new.freparto < curdate() and new.estadoventa = true then
        set new.estadoventa = false;
    end if;
end //
delimiter ;

update venta set estadoventa = true where codigoventa = 3;
select estadoventa from venta where codigoventa = 3;

		-- calcula automáticamente el total de una factura
delimiter //
...
begin
    update facturaventa
    set totalfactura = totalfactura + (new.cantidadprod * new.preciounitario)
    where codigofactura = new.codigofacturafk;
end //
delimiter ;
insert into DetallesFactura (codigoFacturaFK, codigoProductoFK, cantidadProd, precioUnitario)
values (7, 'P5', 3, 10.00);
select totalFactura from FacturaVenta where codigoFactura = 75;

/*-------------------------------------------------------------------------------------------------------------*/
													/*RQF*/
	/*RQF002 Consultar productos en stock*/
select codigoproducto,nombreproducto, descripcionproducto,cantidaddisponible, fentradaproducto, fsalidaproducto, estadoproducto,
...
where stock.cantidaddisponible > 0 and stock.estadostock = true;
	...
	/*RQF003 Inactivar Stock*/
delimiter //
create trigger inactivar_stock
...
begin
    if new.cantidaddisponible <= 0 then
        ...
        where codigostock = new.codigostock;
    end if;
end //
delimiter ;
update stock set cantidaddisponible = 0 where codigostock = 75;

	/*RQF004 Cálculo del SUBtotal de la factura*/
delimiter //
create procedure calcularsubtotalfactura(in codigofactura int)
begin
    declare subtotal double;
    select sum(cantidadprod * preciounitario)
    into subtotal
    from detallesfactura where codigofacturafk = codigofactura;
    select subtotal as 'subtotal factura';
end //
delimiter ;
call calcularsubtotalfactura(7);
