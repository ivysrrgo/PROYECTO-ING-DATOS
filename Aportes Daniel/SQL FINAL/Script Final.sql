...
/*VISTAS PARA VISUALIZACION EN POWER BI*/
...
select * from vista_TotalComprasCliente;
		...
		-- Productos con stock disponible RQF 002
...
-- version 2
create view vistaFacturasPendientes as
select f.codigoFactura,f.docUsuarioFK,f.docClienteFK,f.totalFactura,f.iva,f.fEntrega,f.fReparto,f.estadoPago,f.medioPago,f.direccionEnv,f.precioEnv,f.descripcionFact,f.estadoFactura
from facturaVenta f
where f.estadoPago = 'Pendiente';
select * from vistaFacturasPendientes;

-- detalle factura con subtotal
...
select 
    ...
    p.precioProducto,
    df.cantidadProd,
    (df.cantidadProd * p.precioProducto) as subtotal
from 
    DetallesFactura df
    inner join facturaVenta fv on df.codigoFacturaFK = fv.codigoFactura
    inner join Cliente c on fv.docClienteFK = c.docCliente
    inner join Stock s on df.codigoStockFK = s.codigoStock
    inner join Producto p on s.idProductoFK = p.codigoProducto;

drop view vistaDetalleFactura;

select * from vistaDetalleFactura;

-- consultar los productos que se enviaron según el código de venta registrado. Se visualizarán tanto el nombre de los productos, como su respectiva cantidad

create view vistaProductosEnviados as
select fv.codigoFactura,p.nombreProducto,df.cantidadProd
from facturaVenta fv
inner join DetallesFactura df on fv.codigoFactura = df.codigoFacturaFK
inner join Stock s on df.codigoStockFK = s.codigoStock
inner join Producto p on s.idProductoFK = p.codigoProducto;
    
select * from vistaProductosEnviados ;

-- consultar informacion general por ventas de los clientes

create view vistaResumenClientes as
select c.docCliente, concat(c.nombreCliente, ' ', c.apellidoCliente) as nombreCliente, c.correoCliente,
c.telefonoCliente, c.direccionCliente, sum(df.cantidadProd) as totalProductosComprados,
sum(df.cantidadProd * p.precioProducto) as totalGastado
from Cliente c
inner join facturaVenta fv on c.docCliente = fv.docClienteFK
inner join DetallesFactura df on fv.codigoFactura = df.codigoFacturaFK
inner join Stock s on df.codigoStockFK = s.codigoStock
inner join Producto p on s.idProductoFK = p.codigoProducto
group by 
c.docCliente, nombreCliente, c.correoCliente, c.telefonoCliente, c.direccionCliente;

select * from vistaResumenClientes;

-- veces que un producto ha sido vendido

create view vistaVentasPorProducto as
select 
    p.codigoProducto,
    p.nombreProducto,
    count(df.idDetalle) as vecesVendido,
    sum(df.cantidadProd) as unidadesVendidas
from 
    Producto p
    inner join Stock s on p.codigoProducto = s.idProductoFK
    inner join DetallesFactura df on s.codigoStock = df.codigoStockFK
group by 
    p.codigoProducto, p.nombreProducto;
    
select * from vistaVentasPorProducto;
...
-- Inactivar Stock RQF 003
DELIMITER //
CREATE PROCEDURE InactivarStock(IN p_codigoStock INT)
BEGIN
    UPDATE Stock
    SET estadoStock = FALSE
    WHERE codigoStock = p_codigoStock;
END //
DELIMITER ;
CALL InactivarStock(3);

-- calcular subtotal de una factura RQF 004 y RQF 005

create view vistaResumenFactura as
select 
    fv.codigoFactura,
    c.docCliente,
    concat(c.nombreCliente, ' ', c.apellidoCliente) as nombreCliente,
    ...
    fv.fEntrega,
    fv.fReparto,
    sum(p.precioProducto * df.cantidadProd) as subtotal,
    fv.iva,
    fv.precioEnv,
    sum(p.precioProducto * df.cantidadProd) + fv.iva + fv.precioEnv as totalFacturaCalculado
from 
    facturaVenta fv
    inner join Cliente c on fv.docClienteFK = c.docCliente
    inner join DetallesFactura df on fv.codigoFactura = df.codigoFacturaFK
    ...
    inner join Producto p on s.idProductoFK = p.codigoProducto
group by 
    fv.codigoFactura, c.docCliente, nombreCliente, c.correoCliente, fv.fEntrega, fv.fReparto, fv.iva, fv.precioEnv;

    
drop view vistaResumenFactura;

select * from vistaResumenFactura;

-- Inactivar Factura RQF 007

DELIMITER //
CREATE PROCEDURE InactivarFactura(IN p_codigoFactura INT)
BEGIN
    UPDATE facturaVenta
    SET estadoFactura = FALSE
    WHERE codigoFactura = p_codigoFactura;
END //
DELIMITER ;

CALL InactivarFactura(3);

-- Modificar Info Venta RQF 008
DELIMITER //

CREATE PROCEDURE ModificarVenta(
    IN p_codigoFactura INT,
    IN p_docUsuarioFK INT,
    IN p_docClienteFK INT,
    IN p_totalFactura DOUBLE,
    IN p_iva DOUBLE,
    IN p_fEntrega DATE,
    IN p_fReparto DATE,
    IN p_estadoPago ENUM('Completado','Pendiente','Cancelado'),
    IN p_medioPago VARCHAR(20),
    IN p_direccionEnv VARCHAR(100),
    IN p_precioEnv DOUBLE,
    IN p_descripcionFact TEXT,
    IN p_estadoFactura BOOL
)
BEGIN
    UPDATE facturaVenta
    SET
        docUsuarioFK = p_docUsuarioFK,
        docClienteFK = p_docClienteFK,
        ...
        direccionEnv = p_direccionEnv,
        precioEnv = p_precioEnv,
        descripcionFact = p_descripcionFact,
...
CALL ModificarVenta(
    ...
    'Cra 10 #20-30',  
    5000,            
    'Venta actualizada manualmente', 
    TRUE              
);

-- Consultar productos por venta RQF 009

DELIMITER //

CREATE PROCEDURE ConsultarProductosPorVenta(
    IN p_codigoFactura INT)
BEGIN
    SELECT 
        pr.nombreProducto,
    ...
    INNER JOIN Stock s ON df.codigoStockFK = s.codigoStock
    INNER JOIN Producto pr ON s.idProductoFK = pr.codigoProducto
    WHERE df.codigoFacturaFK = p_codigoFactura;
END //

DELIMITER ;

CALL ConsultarProductosPorVenta(3);

-- Modificar info de un producto RQF 10
...
CALL ModificarProducto(
    ...
    599000, 
    'Versión mejorada con GPS y ECG',
    25,
    TRUE
);

-- consultar cliente por doc RQF011

DELIMITER //
CREATE PROCEDURE ConsultarClientePorDocumento(
    IN p_docCliente INT)
BEGIN
    SELECT *
    FROM Cliente
    WHERE docCliente = p_docCliente;
END //
DELIMITER ;

-- consultar venta por fecha reparto RQF 013 - RQF 014
DELIMITER //
CREATE PROCEDURE FacturasPorFechaReparto(
IN p_fechaReparto DATE)
BEGIN
    SELECT *
    FROM facturaVenta
    WHERE fReparto = p_fechaReparto;
END //

DELIMITER ;

CALL FacturasPorFechaReparto('2025-12-12');

-- consultar cliente por nombre RQF 015
DELIMITER //
...
BEGIN
    SELECT *
    FROM Cliente
    WHERE nombreCliente LIKE CONCAT('%', p_nombreCliente, '%');
END //
DELIMITER ;

CALL ConsultarClientesPorNombre('Luisa');

-- consultar venta por fecha entrega RQF 016
DELIMITER //
CREATE PROCEDURE FacturasPorFechaEntrega(
IN p_fechaEntrega DATE)
BEGIN
    SELECT *
    FROM facturaVenta
    WHERE fEntrega = p_fechaEntrega;
END //

DELIMITER ;

CALL FacturasPorFechaEntrega('2025-12-10');


-- consultar factura por Email RQF 017

DELIMITER //
CREATE PROCEDURE ConsultarFacturasPorCorreoCliente(
    IN p_correoCliente VARCHAR(50)
)
BEGIN
    SELECT fv.*
    FROM facturaVenta fv
    INNER JOIN Cliente c ON fv.docClienteFK = c.docCliente
    WHERE c.correoCliente = p_correoCliente;
END //
DELIMITER ;

CALL ConsultarFacturasPorCorreoCliente('luisavillalba@gmail.com');


-- consultar total facturas de manera descendente RQF 018

CREATE VIEW TotalesFacturasDesc AS
SELECT 
    codigoFactura,
    totalFactura,
    iva,
    precioEnv,
    (totalFactura + iva + precioEnv) AS totalFinal
FROM facturaVenta
ORDER BY totalFinal DESC;

SELECT * FROM TotalesFacturasDesc;


-- cambiar dispo producto RQF 019

DELIMITER //
CREATE PROCEDURE ModificarDisponibilidadProducto(
    IN p_codigoProducto VARCHAR(20),
    IN p_estadoProducto BOOL
)
BEGIN
...
CALL ModificarDisponibilidadProducto('P28', FALSE); 

-- registrar cliente RQF 020
...
CREATE PROCEDURE AgregarCliente(
    ...
    IN p_apellidoCliente VARCHAR(20),
    IN p_correoCliente VARCHAR(30),
    IN p_estadoCliente BOOL,
    IN p_telefonoCliente INT UNSIGNED,
    IN p_direccionCliente VARCHAR(100),
    IN p_tipoCliente ENUM('Natural','Juridico')
)
BEGIN
    INSERT INTO Cliente (
        docCliente,
...
BEGIN
    UPDATE Cliente
    SET estadoCliente = p_estadoCliente
    WHERE docCliente = p_docCliente;
END //
DELIMITER ;

CALL CambiarEstadoCliente(1023456793, FALSE);

-- modificar estado pago factura RQF 023
DELIMITER //
CREATE PROCEDURE CambiarEstadoPagoFactura(
    IN p_codigoFactura INT,
    IN p_estadoPago ENUM('Completado','Pendiente','Cancelado')
)
BEGIN
...
CALL CambiarEstadoPagoFactura(5, 'Completado');

-- cambiar tipo de cliente RQF 024
DELIMITER //
...
CALL CambiarTipoCliente(1023456793, 'Juridico');

-- consultar usuario por documento RQF 025
DELIMITER //
CREATE PROCEDURE UsuarioPorDocumento(
    IN p_docUsuario INT
)
BEGIN
    SELECT *
    FROM Usuario
    WHERE docUsuario = p_docUsuario;
END //
DELIMITER ;

CALL UsuarioPorDocumento(105234591);

-- clientes con más productos comprados RQF 026

CREATE VIEW ClientesMasProductosComprados AS
SELECT 
    c.docCliente,
    c.nombreCliente,
    c.apellidoCliente,
    SUM(df.cantidadProd) AS totalProductosComprados
FROM Cliente c
JOIN facturaVenta fv ON c.docCliente = fv.docClienteFK
JOIN DetallesFactura df ON fv.codigoFactura = df.codigoFacturaFK
GROUP BY c.docCliente, c.nombreCliente, c.apellidoCliente
ORDER BY totalProductosComprados DESC;


SELECT * FROM ClientesMasProductosComprados;

-- consultar facturas por estado de pago RQF 027

DELIMITER //
CREATE PROCEDURE ConsultarFacturasEstadoPago(
    IN p_estadoPago ENUM('Completado','Pendiente','Cancelado')
)
BEGIN
    SELECT *
    FROM facturaVenta
    WHERE estadoPago = p_estadoPago;
END //
DELIMITER ;

CALL ConsultarFacturasEstadoPago('Pendiente');

-- editar info Usuario RQF 028
...
CREATE PROCEDURE ModificarUsuario(
    ...
    IN p_apellidoUsuario VARCHAR(20),
    IN p_correoUsuario VARCHAR(50),
    IN p_estadoUsuario BOOL,
    IN p_telefonoUsuario INT UNSIGNED,
    IN p_direccionUsuario VARCHAR(100),
    IN p_tipoUsuario ENUM('Administrador','Empleado'),
...
-- consultar Productos por precio RQF 032

DELIMITER //
CREATE PROCEDURE BuscarProductoPorPrecio(
    IN p_precio DOUBLE
)
BEGIN
    SELECT *
    FROM Producto
    WHERE precioProducto = p_precio;
END //
DELIMITER ;

CALL BuscarProductoPorPrecio(1300000);


-- consultar producto por nombre RQF 035 
...
CALL BuscarProductoPorNombre('gafas');

-- consultar productos por caracteristicas RQF 036
DELIMITER  // 
...
-- agregar nuevos usuarios RQF 037
...
BEGIN
    INSERT INTO Usuario (
        docUsuario, nombreUsuario, apellidoUsuario, correoUsuario, estadoUsuario, 
        telefonoUsuario, direccionUsuario, tipoUsuario, nickname
    ) VALUES (
        p_docUsuario, p_nombreUsuario, p_apellidoUsuario, p_correoUsuario, p_estadoUsuario, 
        p_telefonoUsuario, p_direccionUsuario, p_tipoUsuario, p_nickname
    );
END //
DELIMITER ;
